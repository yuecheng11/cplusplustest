...

{
  ...
  pair<CMyString, string> sz[] = {pair<CMyString, string>("beijing", "huabei"), pair<CMyString, string>("shanghai", "huazhong")};

  map<CMyString, string> obM(sz, sz+2);

  obM.insert(pair<CMyString, string>("guangzhou", "huanan"));

  obM["wuhan"] = "huazhong";

  obM["xiamen"] = "huanan";

  obM["shanghai"] = "huanan";  //上海不是华南, 错了

  obM["shanghai"] = "huadong";  //更正上海为华东
  ...
  map<CMyString, string>::iterator itM = obM.find("xiamen");
